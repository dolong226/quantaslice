"""Kiểu dữ liệu cấp hệ thống/runtime — khác với ``types.py`` (domain
types cho bài toán phân bổ), module này chứa cấu hình vận hành và
"khung" trạng thái tổng hợp dùng cho dashboard/demo.

Tách riêng khỏi ``types.py`` để: (1) mỗi file giữ dưới 300 dòng theo
chuẩn implementation style, (2) tách bạch rõ "domain type" (mọi package
đều cần) khỏi "system type" (chủ yếu pipeline/dashboard/cli cần).
"""
...
class SimulationFrame:
    """Một "khung" trạng thái toàn hệ thống tại một mốc thời gian, dùng
    cho dashboard/visualization. Package quantum/ai không cần biết đến
    kiểu dữ liệu này — nó chỉ được ``pipeline`` tạo ra và publish."""
